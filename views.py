...
def login():
    # if we not check this statement then after login when we go to login page it will always ask us to login in again
    if current_user.is_authenticated:
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            # need to pass the current user because it doesnot know which user is going to logged in
...
@app.route('/logout')
# no need to pass any argument in logout_user it already know which user is currently logged in
def logout():
    logout_user()
    return redirect(url_for('home'))
...
def account_dashboard():
    image_file = url_for('static', filename='css/pics/user.jpg')
